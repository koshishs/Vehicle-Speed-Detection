...
class ObjectTracker:
    ...
    # SPEEED FUNCTION
    def getsp(self, id):
        # Check if the id exists in the self.s dictionary and if the corresponding value is not equal to zero
        if (self.s[0, id] != 0):
            # Calculate the speed by dividing 80.15 by the time difference self.s[id]
            s = 85.15 / self.s[0, id]
        else:
            # If the above conditions are not met, set the speed to 0
            s = 0
